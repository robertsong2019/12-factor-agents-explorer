...
3. ⬜ Next dev targets: **temporal past-perfect duration 家族 14q（"how long had I been X when Y"——state-start 锚 + 单位推断，C482 遗留最大 coherent 族）+ order 家族 10q（N 锚排序 + listing judge）** / **multi_session duration_sum 过求和（social media 59vs17、Hawaii 90vs15）+ driving 计划/事实区分（C483 遗留）** / preference 30q 0.000 结构性缺口取证（先确认检索侧 vs 答案侧，evhit 0.567 六类最低）/ 桶 B 弃权（C448 熵门答案侧推广，随 C484）/ full-500 rerun ~C484（验证 temporal 0.323 上 reference + counting 收编后 multi_session 0.045 持平）/ 8月底双首跑① GraphRAG-Bench Novel sample_100 retrieval_eval（唯一阻塞=`ollama pull qwen2.5:7b`，本机未装 ollama）/ cat5+多跳残余→LLM judge 实测（机制 C462-465 全就位，等 ollama；#074 已证 ssa 检索侧就绪）/ MCP registry publish / OpenClaw plugin。✅ 已落：ssa 答案侧（C475）/ counting forms（C477+C483）/ full-500 reference（C481）
