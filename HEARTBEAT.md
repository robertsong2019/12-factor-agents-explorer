...
- **四项目总计**: 11788 tests ✅（amg 9647 + sot 571 + atc 1570）
- **全项目总计**: ~20661 tests（08-20 KO 口径：amg 9647；obs 222/nano 1156/prompt-mgr 283 沿用修正台账）
- **零回滚率**: amg **297天** 🏆（KO 日历链：08-18=295 / 08-19=296 / 08-20=297，会话漂移勿沿用；C482/C483 会话自报 297 与 KO 链一致）/ acs 200天 🏆
...
3. ⬜ Next dev targets: **past-perfect duration 家族 — Research #077 ✅ 08-20（真人口 7 非排 14；原型 7/7 基线 0：跨会话 ago 算术 + 嵌套 tenure 减法 + 负存在弃权 + 两阶段交叉排除选锚；v2 纯 tenure/did-before 另 +7q）→ C486 生产接入首选** / **order 家族 9q — Research #078 ✅ 08-20（原型 9/9 基线 0：fresh-priority 锚定 + 子句级 planning + 排序渲染；姊妹成对家族 29q 中 21 错 = +4.2pp → C487）→ C486 与 #077 并列** / multi_session duration_sum 过求和（social media 59vs17、Hawaii 90vs15）+ driving 计划/事实区分（C483 遗留）** / preference 30q 0.000 结构性缺口取证（先确认检索侧 vs 答案侧，evhit 0.567 六类最低）/ 桶 B 弃权（C448 熵门答案侧推广，随 C484）/ full-500 rerun ~C486（验证 temporal 0.323 上 reference + counting 收编后 multi_session 0.045 持平）/ 8月底双首跑① GraphRAG-Bench Novel sample_100 retrieval_eval（唯一阻塞=`ollama pull qwen2.5:7b`，本机未装 ollama）/ cat5+多跳残余→LLM judge 实测（机制 C462-465 全就位，等 ollama；#074 已证 ssa 检索侧就绪）/ MCP registry publish / OpenClaw plugin。✅ 已落：ssa 答案侧（C475）/ counting forms（C477+C483）/ full-500 reference（C481）
