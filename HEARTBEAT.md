...
- **四项目总计**: 11909 tests ✅（amg 9768 + sot 571 + atc 1570）
- **全项目总计**: ~20782 tests（08-21 KO 口径：amg 9768 实测；obs 222/nano 1156/prompt-mgr 283/skill-test 20/mcpt 38 沿用修正台账）
- **零回滚率**: amg **298天** 🏆（KO 日历链：08-18=295 / 08-19=296 / 08-20=297 / 08-21=298，会话漂移勿沿用；C482→C489 全 keep 连续 8 轮）/ acs 200天 🏆
...
3. ⬜ Next dev targets: **full-500 rerun（逾期头号：reference 0.204 早于 C482-C489 六连，temporal 0.474 + order/pairwise 增益未入台账，投影 overall ~0.24+）** / 29-family 残余 8 unanchored fallthroughs（Dell XPS/tomatoes since-form 等，需更宽锚动词映射或 topic-scope 锚）/ temporal_arith first-kind 弃用评估（C489 pairwise 已涵盖且更准，5 个 TA-correct 同答验证）/ binoculars 事件锚取证（3w vs 2w，detail 未透出 retrieval）/ multi_session duration_sum 过求和（social media 59vs17、Hawaii 90vs15）+ driving 计划/事实区分（C483 遗留）/ preference 30q 0.000 结构性缺口取证（先确认检索侧 vs 答案侧，evhit 0.567 六类最低）/ 桶 B 弃权（C448 熵门答案侧推广）/ 8月底双首跑① GraphRAG-Bench Novel sample_100 retrieval_eval（唯一阻塞=`ollama pull qwen2.5:7b`，本机未装 ollama）/ cat5+多跳残余→LLM judge 实测（机制 C462-465 全就位，等 ollama）/ MCP registry publish / OpenClaw plugin。✅ 已落：ssa 答案侧（C475）/ counting forms（C477+C483）/ full-500 reference（C481）/ past-perfect duration（C486/487）/ order 家族 + pairwise which-first（C488/489）

## 上次检查
- **Knowledge org: 2026-08-21 02:00** — Integrated C484-C489（amg 9647→**9768** verified：pytest collect 实测 9768 ✓ + git 311eeed/b9815ee/82ce1ff 在库 + 项目仓 experiments.tsv 链 9647→9682→9694→9733→9768 完整；day counter 298=KO 链 08-18:295→08-21:298）。**temporal-133 五连：0.271→0.323→0.376→0.444→0.474**（36→63/133，全部零翻转）；C484/C485 标 needs_verification 但 C486 pre-arm 0.323 精确复现 → 非回滚非扰动。四项目 11909 / 全项目 ~20782（新增 skill-test 20/mcpt 38 入台账口径）。MEMORY：Active Theme 前插 484-489 arc + 表格刷新 + arc 节改名 475-489 + Next 重写（full-500 rerun 上位头号）+ insight #245（原始时间精度截断=静默排序 bug；弃权也是答案）；HEARTBEAT：标题/计数/近期活动节/Next dev targets 全刷新。
