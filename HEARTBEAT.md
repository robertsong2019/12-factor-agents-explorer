...
3. ⬜ Next dev targets: **嵌入 side-channel 生产化（头号，#083 双档定案：model2vec 静态=fast 档 / fastembed MiniLM=质量档，form-gated switch 非 RRF，import 探测式，预期 ssa evhit 0.929→~1.0 / preference hit 0.567→~0.87）** / **在飞 C501（08-23 02:00 工作树：amg_bench_quality.py +96 未提交 + test_role_answer/test_optimization +10 tests——勿碰，待会话自决）** ~~driving 计划-事实区分~~（#085 勘误 STALE：C499 已 exact，C497b 签名已修）/ multi_session 下一簇（C500 后仍 111 错：count/distance/percentage；36b9f61e 类别求和、91b15a6e minimum-amount 形式）/ C498b llm-judge 口径弃权计分（需 ollama）/ 8月底双首跑① GraphRAG-Bench Novel sample_100 retrieval_eval（唯一阻塞=`ollama pull qwen2.5:7b`，本机未装 ollama）/ cat5+多跳残余→LLM judge 实测（机制 C462-465 全就位，等 ollama）/ MCP registry publish / OpenClaw plugin / 博客候选 "the date was in the text all along" + "bridging the vocabulary gap without an LLM"（#083 素材）。✅ 已落：full-500 reference（C481+C492+**C499 exact 0.316**）/ 锚纪律两连（C490/C491，multi_session 0.120）/ TA first-kind 退役（C493）/ pairwise v2 三连（C494-496，temporal 0.571）/ neither 族（**C497 ECM，temporal 0.602**）/ preference 诚实弃权（**C498，29/30 abstain**）/ item_total（**C500，multi 0.166**）/ #082/#083 研究（嵌入通道就绪）
