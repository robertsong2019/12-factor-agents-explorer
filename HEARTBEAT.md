...
3. ⬜ Next dev targets: **C497 neither 族（头号，Research #082 ✅ 取证+原型 4/4 oracle + 零劫持 gate 4/500 已验证——ECM 五决策直接移植，预期 temporal-133 0.571→0.602）** / driving 计划-事实区分（C483 遗留，GT 15 vs 19）/ full-500 刷新债（C492 exact 0.284 后又攒 C493-C497，C497 后再刷）/ preference form-gate 诚实弃权（#080，~20 行）+ 嵌入 side-channel（**#083 研究完成 ✅：MiniLM 打通 preference 检索桥 @5 0.60→0.87，双档选型定案 model2vec 静态/fastembed 神经，form-gated switch 非 RRF → C498 生产化候选**）/ 判分器 GT word-number 归一化 / 桶 B 弃权（C448 熵门答案侧推广）/ 8月底双首跑① GraphRAG-Bench Novel sample_100 retrieval_eval（唯一阻塞=`ollama pull qwen2.5:7b`，本机未装 ollama）/ cat5+多跳残余→LLM judge 实测（机制 C462-465 全就位，等 ollama）/ MCP registry publish / OpenClaw plugin。✅ 已落：full-500 reference（C481 + C492 exact 0.284）/ 锚纪律两连（C490/C491，multi_session 0.120）/ TA first-kind 退役（C493）/ pairwise v2 三连（C494-496，temporal 0.571）/ neither 族研究（#082 取证五墙+ECM 原型）/ **嵌入 side-channel 研究（#083：三引擎实测，RRF 负结果，tie-break 伪影事故入档）**
