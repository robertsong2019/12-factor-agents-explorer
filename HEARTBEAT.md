...
3. ⬜ Next dev targets: **⓪ judge_semantic() A/B（#090 判分器落地候选：C519 解锁 3 题 + kupdate 12 rescue 题 + C525 1a8a66a6 类 containment 口径 win 是天然评测题，llm 标签当 oracle）** / **窗口组成 census（C525 队列：dump 预算截断前完整 ranked list + GT 行 rank 分布；92 题分层 rank 13-N vs 未检出；17 全漏题 seed 召回失败归因）** / answer-gate 非数量型 echo（~200 wrong 同病理不同触发面，需新行选择信号）/ ssu 34-wrong + speaker_recall 26-wrong 取证（C517 队列遗留）/ entropy gate 6 题误杀 / POST --sidechannel 臂刷新（C517 遗留）/ 三解锁题 answer-face 转述失配（25e5aa4f/488d3006，高风险）/ **博客勘误节（e9dd6a4 的 6.1× 已证伪为 1.02×）** / 8月底双首跑① GraphRAG-Bench Novel sample_100（阻塞=ollama pull qwen2.5:7b）/ cat5+多跳残余→LLM judge 实测（等 ollama）/ MCP registry publish / OpenClaw plugin / 博客候选 "the question is the join condition"（#086）+ "two directions, one word"（#087）+ "the wall was four walls"（#084）+ "presupposition failure is an answer"（C513/C514 弃权弧线）+ "the judge is the mirror"（#090 判分↔作答镜像律）。✅ 已落：**C525 ku recency session-scope answer face（census +2/−0，官方待收债；C524 spin-off 兑现）** + C524 latest-number-wins census RECORD-NEGATIVE（C523 top-has-number 守卫证实承重，方向关闭）+ C520 risk_coverage_report + C521 enum_count event + C522 bigram 负结果&官方刷新 0.454 + C523 quant_rerank 0.476（+11/−0）
