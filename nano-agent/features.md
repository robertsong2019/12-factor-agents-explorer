...
### Sequence, Integrity & Events (Round 20 — 2026-08-17)
- [x] **F67**: Sequence protocol — `__len__`/`__iter__`/`__getitem__`/`__contains__` making Memory a first-class Python sequence: negative indices, slices, snapshot-safe iteration, `MemoryEntry in mem` (content equality) & exact `str in mem`; unlocks enumerate/comprehensions/`any()` composition ✅ 2026-08-17
- [x] **F68**: `Memory.copy()` + `Memory.content_hash(include_archived?)` — deep copy (data-only: no persistence path, no subscribers; pins/annotations/LRU metadata survive) for undo safety before destructive ops; SHA-256 canonical-JSON state fingerprint (order-sensitive, timestamps included; archived entries opt-in via flag) for tamper detection & snapshot comparison ✅ 2026-08-17
- [x] **F69**: `Memory.subscribe(event, callback) -> token` / `unsubscribe(token)` — reactive hooks on add/remove/update (incl. batch variants); callback `(event, entry, index)`; observer exceptions swallowed (never break core ops); maintenance ops (resize/forget/clear/LRU-trim) intentionally silent ✅ 2026-08-17
