...
[![Tests](https://img.shields.io/badge/tests-8794-brightgreen)]()
...
#### `FastAppendQueue(mg, *, auto_flush_threshold=100, consistency_mode="eventual")` (Cycle 425-427)

System-1/System-2 双进程写入路径。System-1 热路径 O(1) append + 关键词搜索；System-2 冷路径 flush + 去重 + 图集成。支持 peek/flush_and_consolidate/is_healthy/peak_buffer_size。4 种一致性模式。

#### `knowledge_freshness_report(*, group_by=None) -> dict` (Cycle 426)

FAMA 感知图级新鲜度诊断。5 级时间桶 (fresh/recent/aging/stale/decayed) + 加权评分 + 分组分解 + 最旧/最新节点列表 + 可执行建议。

#### `extract_from_text(text, *, deduplicate=True) -> dict` (Cycle 428)

零依赖规则式 KG 构建。句子分割 → 大写实体检测 (1-3 词) + 引号术语 → 7 种关系模式 (is_a/works_at/created/located_in/has/part_of/built) → 去重。返回 nodes_created/edges_created/entities/relations。

#### `graphrag_query(question, *, max_hops=2, top_k=10) -> dict` (Cycle 429)

GraphRAG 子图检索。停用词过滤关键词提取 → 种子节点匹配 (label LIKE + tag JSON) → BFS 双向遍历 → 关键词×中心性×跳数衰减排名 → top_k + LLM 上下文字符串。

#### `graphrag_explain(question, result, *) -> dict` (Cycle 430)

逐查询 GraphRAG 诊断。关键词分解 (匹配/未匹配 + 匹配类型) + 逐节点得分分解 + 遍历路径重建 + 覆盖率分析 + 人类可读解释 + 改进建议。

#### `graphrag_coverage_report() -> dict` (Cycle 431)

全局 KG 检索健康诊断。标签覆盖率 + 标签覆盖率 + 平均标签数 + 关键词索引 + 孤儿率 + 度数统计 + 可匹配性分级 (high/medium/low) + 复合健康分 + 稀疏节点检测 + 上下文感知建议。GraphRAG 全流水线完结: extract_from_text → graphrag_query → graphrag_explain → graphrag_coverage_report。
