...
search_graphrag) across configurable scale tiers.
...
class BenchHarness:
    ...
    def _bench_multi_hop(self, mg: MemoryGraph, queries: list[str]) -> float:
        """Measure average search_graphrag (multi-hop retrieval) latency (ms)."""
        latencies = []
        for q in queries[:10]:
            t0 = time.perf_counter()
            try:
                mg.search_graphrag(q, mode="local", limit=5)
            except Exception:
                pass
            latencies.append((time.perf_counter() - t0) * 1000)
        return statistics.mean(latencies) if latencies else 0.0
    ...
    def _run_single_scale(self, n: int) -> BenchmarkResult:
        ...
        multi_hop_ms = self._bench_multi_hop(mg, queries)
