...
### Active Theme
Autoresearch 方法论实践 - **连续160天零回滚率** 🏆。06-28 凌晨 **agent-context-store +223 tests (cycles 172-180, 9 cycles!)**: knowledge_graph_core_periphery (Borgatti-Everett decomposition) + knowledge_graph_modularity (LPA + resolution param) + knowledge_graph_betweenness_centrality (Brandes' algorithm σ shortest-path) + quality_entropy_report (Shannon) + quality_drift_detector (version-history temporal drift) + store_hotspot_report + store_churn_report (metabolic state) + store_compression_potential (trigram near-dup) + content_readability_report + tag_suggestion_engine + content_embedding_proxy + tag_temporal_drift。06-27 23:10 **agent-memory-graph +30 tests (cycle 169): diffusion_retrieve()** — ExpGraph-inspired Personalized PageRank! Research-to-production <24h。06-27 凌晨 agent-context-store cycles 170-171 (+55): assortativity + small_world + leaderboard + correlation + memory_efficiency + rhythm。**管线里程碑**: agent-context-store 三大分析管线全面突破 — Graph **12层** (structure→importance→connectivity→extremes→fragility→communities→robustness→assortativity→small_world→modularity→core_periphery→betweenness), Quality **12层** (diagnose→batch→plan→batch_plans→simulate→diff→distribution→leaderboard→correlation→entropy→drift_detector→readability), Store **13层** (snapshot→trend→forecast→anomalies→seasonality→growth_model→velocity→memory_efficiency→rhythm→hotspot→churn→compression→embedding_proxy)。**最高优先级**: README → npm publish (四项目: agent-memory-graph 1554 + agent-context-store 2253 + structured-output-toolkit 507 + agent-task-cli 986 = **5300 tests**)。

### 🔑 最新关键洞察 (06-25~20)
- **Agentic Graph Memory 2026: Dual-Route + RL + Reconstruction** ✅ (06-25 晚): [笔记](catalyst-research/exploration-notes/2026-06-25-agentic-graph-memory-2026-dual-route-rl-reconstruction.md) ✅ ~300行可运行 TypeScript DualRouteRetriever (17/17 assertions pass). **12 papers/systems synthesized**: Mnemis(MS ACL 2026, arXiv:2602.15313, dual-route S1 similarity+S2 hierarchical global selection, 93.9 LoCoMo / 91.6 LongMemEval-S, built on Graphiti, re-ranking NOT significant → structure > ranking) + Graph-R1(ICML 2026, arXiv:2507.21892, RL-trained think-query-retrieve-rethink on knowledge hypergraphs, GRPO/PPO/REINFORCE++ on Qwen2.5-3B 4×48GB GPU, outperforms GraphRAG on 6 benchmarks, 3B+RL > 70B+prompt) + MRAgent(ICML 2026, arXiv:2606.06036, Cue-Tag-Content graph, active reconstruction ≠ static retrieval, iterative explore-prune-backtrack, +23% on LoCoMo/LongMemEval) + MemAdapter(arXiv:2602.08369, cross-paradigm alignment 13min single-GPU, generative subgraph retrieval) + SCOUT-RAG + HORMA + Google Agentic RAG + Mem0 State of Memory 2026 + Atlan Best Frameworks 2026 + LLMs+Graphs survey + 20 Advanced RAG Types + Awesome-AI-Memory. **5核心概念**: (1)Three paradigms converging — hierarchical(Mnemis) / RL(Graph-R1) / reconstruction(MRAgent); (2)Structure > ranking — Mnemis proved re-ranking ceiling, hierarchy is the lever; (3)Cue-Tag-Content abstraction — decouple what(tag) from whom(cue) before retrieving content; (4)RL training is future but architecture-first works today — Mnemis/MRAgent competitive without training; (5)npm market gap quantified — Mnemis needs Neo4j, Graph-R1 needs GPU, MRAgent is research-only, Mem0 dropped graph. **5关键洞察**: (1)Static retrieve-then-reason is dead — all 3 papers independently reject it; (2)Complementarity is key — Mnemis S1≈85 alone, S2≈88 alone, combined=93.9; (3)LLM should guide retrieval decisions at each step — not just before/after; (4)Hypergraph(n-ary relations) > binary edges for complex knowledge; (5)agent-memory-graph uniquely positioned to implement all 3 patterns in TS/SQLite. **下一步**: hierarchical category layer ~100行+20tests / Cue-Tag-Content traversal API ~60行+15tests / README cite Mnemis+Graph-R1+MRAgent as architecture inspiration
- **KV Cache as Agent Working Memory: From Inference Optimization to Cross-Agent Collaboration** ✅ (06-26 晚): [笔记](catalyst-research/exploration-notes/2026-06-26-kv-cache-as-agent-working-memory-inference-to-collaboration.md) ✅ ~300行可运行 TypeScript KVCacheAwareMemoryManager (cache temperature + workflow-aware eviction + cross-agent segment reuse + plan template extraction + health report, 5/5 assertions pass). **15+ papers/systems synthesized**: KVFlow(NeurIPS 2025, arXiv:2507.07400, Agent Step Graph + steps-to-execution eviction, 2.19× speedup) + KEEP(DAC 2026, arXiv:2602.23592, Static-Dynamic memory construction + Multi-hop recompute + Layer-balanced loading, 2.68× speedup) + Segment-Level KV Cache Sharing(ICLR 2026 submission, OpenReview kgzBkyqg6Z, breaks prefix barrier with fine-grained semantic segments + RoPE correction) + TokenDance(arXiv:2604.03143, All-Gather pattern for multi-agent rounds, cites OpenClaw explicitly) + SideQuest(NVIDIA, arXiv:2602.22603, model-driven parallel auxiliary thread eviction, 56-65% token reduction) + KVCOMM(NeurIPS 2025, cross-context KV offset alignment via anchor pool, 70% reuse rate) + Continuum(arXiv:2511.02230, KV cache TTL for multi-turn agents + program-level FCFS) + Agentic Plan Caching(NeurIPS 2025, arXiv:2506.14852, plan template extraction + keyword matching, 50.31% cost ↓ 27.28% latency ↓) + AgenticCache(arXiv:2604.24039, plan-transition 2-gram cache, 65% latency ↓) + IntentKV(arXiv:2606.09916, cross-turn intent-aware multi-query retention, 20.7% prefix-hit @ 88K) + Tokencache(KV-cache-centric multi-agent serving, DualRadixTree + CoW) + CacheSlide(FAST 2026, cross-position-aware reuse) + ScaleSim(large-scale simulation memory) + SIGARCH 2026(computer architecture view: Missing Piece 1 Agent Cache Sharing Protocol + Missing Piece 2 Agent Memory Access Protocol) + LLM Agent Memory Survey(preprints.org March 2026, unified representation includes KV cache) + Semantic Sponsorship(arXiv:2604.11288, dormant token analysis + safety cliff at 90% compression) + Persistent Q4 KV Cache(arXiv:2603.04428, edge agent disk-backed KV) + Awesome KV Cache Optimization(github.com/jjiantong). **5核心概念**: (1)KV Cache IS Agent Working Memory — not just inference optimization but semantic substrate of agent state (KEEP: 'critical carrier of agents\' working memory'); (2)Prefix Barrier Breaking — from rigid prefix matching → semantic segment sharing → collective All-Gather patterns (3 independent approaches converging); (3)Model-Driven Eviction — LLM-as-garbage-collector (SideQuest parallel auxiliary thread) > heuristic rules (H2O/SnapKV); (4)Workflow-Aware Cache Management — KVFlow Step Graph + steps-to-execution + Continuum TTL + APC plan templates; (5)SIGARCH Cache Coherence for Agents — Missing protocols that agent-memory-graph CRDT+HLC+trust directly addresses. **5关键洞察**: (1)External memory (agent-memory-graph) ↔ Internal memory (KV cache) are two layers of same problem — consolidation_pipeline = external SideQuest; (2)Prefix Barrier = BM25 Barrier — same evolution path exact→fuzzy→semantic, Segment-Level RoPE correction is position-encoding tax; (3)KVFlow Step Graph maps to Workflow Memory — record_outcome should output reuse_proximity metadata for KV eviction; (4)Plan Caching proves trajectory-level reuse works — 50% cost reduction with plan templates = inference-layer analog of Workflow Memory + Skill Discovery; (5)npm ecosystem zero TS-native KV cache agent memory bridges — Python has 10+ systems (vLLM/KVFlow/KEEP/Tokencake), TypeScript has zero = greenfield opportunity. **下一步**: agent-memory-graph cache_temperature() API ~40行+10tests / plan template extraction ~60行+15tests / README 'KV Cache Aware' section + 'cache coherence protocol for agent memory' positioning citing SIGARCH 2026 / future agent-kv-bridge package ~200行
- **Knowledge Graph Embeddings for Agent Memory: Structure-Aware Retrieval** ✅ (06-26 晚): [笔记](catalyst-research/exploration-notes/2026-06-26-knowledge-graph-embeddings-structure-aware-retrieval-agent-memory.md) ✅ ~300行可运行 TypeScript (TransE score + RotatE score + TransETrainer SGD + KGEEnhancedRetriever 4-way fusion + Demo, 3/3 tests pass). **15 papers/systems synthesized**: ReaLM(WWW 2026, arXiv:2510.09711, RVQ bridges KGE→LLM tokens, RotatE+LoRA SOTA on link prediction) + GraphQ-LM(ICLR 2026, scale-free graph tokenization via RVQ) + GraphRAG-R1(WWW 2026, arXiv:2507.23581, process-constrained RL prevents reward hacking, +38.37% on LightRAG) + SeedER(arXiv:2605.23753, seed-and-expand retrieval: dense finds entrance, graph expansion finds path, RL-guided local decisions, O(log|V|) vs Ω(|V|) for dense) + RoE(WWW 2026 Oral, unified retrieval+generation via graph exploration) + GNN-RAG(ACL 2025, GNN retriever + LLM reasoner) + RotatE(ICLR 2019, complex space rotation, symmetry/antisymmetry/inversion/composition) + TransE(NeurIPS 2013, h+r≈t translation) + ICML 2026 theoretical limitations (Ω(|V|) for relation-tracing graphs) + HYPER(ICLR 2026, foundation model for inductive LP) + KG-BiLM(WWW 2026, bidirectional LM KGE) + Bayesian-Guided Continual KGE(WWW 2026, lifelong embedding) + SAGE(arXiv:2602.16964, structure-aware graph expansion) + HyperRAG(WWW 2026, n-ary facts) + Comprehensive KGE Survey(MDPI). **5核心概念**: (1)KGE四范式 Translation/Rotation/Neural/Hypercomplex — TransE 80%价值20%复杂度; (2)ReaLM RVQ桥接 — KGE→discrete code tokens→LLM vocab, 但逆命题更有价值: KGE作为检索重排序信号不需LLM; (3)SeedER两阶段 — dense retrieval找入口+graph-aware RL expansion找路径, 统一人类记忆检索; (4)GraphRAG-R1过程约束 — 检索次数上下界防reward hacking; (5)理论限制 — ICML 2026证明KGE需多信号融合, 不能单独使用. **5关键洞察**: (1)Text embedding编码"what", KGE编码"where" — 结构等价性是第4种检索信号; (2)TransE 80-20法则 — h+r≈t核心仅50行, 训练<1秒(1000节点), 64维×节点=640KB; (3)SeedER "Dense finds entrance, Graph finds path" 是agent memory检索统一框架 — 替换search_hybrid暴力BFS; (4)KGE差异化为npm唯一 — Mem0/Zep/Letta均无KGE, agent-memory-graph将成为Text+BM25+Graph+**KGE**四路融合; (5)集成路径清晰: Phase1 TransE score ~30行 → Phase2 Trainer ~60行 → Phase3 search_hybrid kge_weight ~40行 → Phase4 SeedER expansion ~100行. **下一步**: P1 kge_score()工具函数~30行 / P2 TransETrainer~60行+SQLite边表训练 / P3 search_hybrid()添加kge_weight参数 / README "Structure-Aware Retrieval via KGE" + cite ReaLM+SeedER
- **Compositional Agent Memory: 3-Layer Unification** ✅ (06-20 晚): MemRL(arXiv:2601.03192) runtime RL Q-value scoring on episodic memory + MemAct(arXiv:2510.12635) memory-as-action DCPO RL + AgentFold(arXiv:2510.24699) proactive context folding (30B beats 20x larger, 500+ turns) + E-mem(ICML 2026) episodic context reconstruction (multi-agent master-assistant, non-destructive) + SSGM(arXiv:2603.11768) drift taxonomy (semantic/procedural/goal) + Governed Memory(arXiv:2603.17787) 99.6% fact recall dual memory model + MemoryArena(ICML 2026) recall≠agency + MLMF(arXiv:2603.29194) adaptive retrieval gating 5.1% false memory. **核心洞察**: (1)Memory management从系统问题变为学习问题; (2)Frozen LLM + evolving memory是stability-plasticity共识架构; (3)Episodic在被重建非仅检索; (4)生产需要governance+drift detection; (5)Benchmark从recall→agency. ~250行可运行 TS CompositionalMemory (episodic Q-value + semantic consolidation + procedural skills + AgentFold folding + SSGM drift detection) 4/4 pass. **下一步**: agent-memory-graph Q-value scoring ~60行 / drift detection ~50行 / README 3-layer positioning
...
### ⚠️ 关键发现
- **agent-memory-graph 已突破 1554 tests** (06-27): 06-27 cycle 169 **diffusion_retrieve()** (ExpGraph PPR, +30 tests, research→production <24h). 06-26 cycles 166-168 (+41: memory_lifecycle_report + memory_access_pattern + memory_health_score composite KPI). 06-24 cycles 150-151 (+176: Graph Reasoning 4 API + Adaptive Retrieval). 350+ APIs. 06-19 evening 3-cycle 1213→1250 (+37: memory_decay/neighborhood_agreement/proximity/tag_induced_subgraph/annotate) + 06-19 code lab 4-cycle 1250→1307 (+57: Workflow Memory 14 APIs — AWM ICML 2025 Procedural Memory). **npm生态唯一图分析+向量+BM25+Adaptive Fusion+RL Memory+CRDT多Agent合并+语义Consolidation+Workflow Memory八合一**。
- **agent-context-store 已突破 2253 tests** (06-28): 06-28 cycles 172-180 (+223! 9 cycles in 24h): core_periphery(Borgatti-Everett) + modularity(LPA) + betweenness_centrality(Brandes') + entropy(Shannon) + drift_detector + hotspot + churn + compression + readability + tag_suggestion + embedding_proxy + temporal_drift → 06-27 cycles 170-171 (+55) → 06-26 cycles 164-169 (+94). **三大管线 37 层**: Graph 12 / Quality 12 / Store 13. **API 总量 500+ methods**.
- **structured-output-toolkit 已突破 507 tests** (06-25): 478→507 (+29: batchValidate + listVersionSummaries + diffChain + batchValidationReport + exportSnapshot + hasMigrationEdge).
- **agent-task-cli 已突破 986 tests** (06-27): 967→986 (+19: F150 Cache.replace + F151 Cache.retain + F152 Storage.rename + merge cleanup).
...
### [06-21 新研究] Agent Memory Security: Poisoning, Integrity & Production Defense
- [x] **Agent Memory Security 深度研究** ✅ 2026-06-21 晚 — [笔记](catalyst-research/exploration-notes/2026-06-21-agent-memory-security-poisoning-integrity-defense.md) ✅ MemoryIntegrityGuard ~250行可运行 TypeScript (4-layer defense: write-time provenance + retrieval-time trust + behavioral monitoring + quarantine/recovery, 8/8 assertions pass)
  - **15 papers/systems synthesized**: arXiv:2604.16548 Memory Lifecycle Framework (6-phase chain) + arXiv:2606.04329 Systematic Study (9 vulnerabilities, 4 write channels, 6 attack classes, MPBench) + AgentPoison(NeurIPS 2024, ≥80% ASR <0.1% poison) + MINJA(ICLR 2026, >95% ASR) + InjecMEM(61.4% retrieval, 76.6% conditional) + eTAMP(cross-session web) + A-MemGuard(ICLR 2026, consensus validation + dual-memory) + VerificAgent(domain-specific verification) + TierMem(ICLR 2026 Workshop, provenance-aware tiered) + Portable Agent Memory(arXiv:2605.11032, BLAKE3+Ed25519+DAG) + MemFlow(arXiv:2603.15125, control flow integrity) + OWASP Agent Memory Guard + Morris-II Worm + Implicit Memory Bombs + LangGraph Checkpoint RCE
  - **5核心概念**: (1)Memory Lifecycle Attack Chain (Write→Store→Retrieve→Execute→Share→Forget); (2)Attack Taxonomy 9 vulnerabilities/4 channels/6 classes; (3)4-Layer Defense (Write provenance → Retrieval trust → Behavior monitor → Recovery); (4)Empirical ASR data (80-95% across studies); (5)Provenance DAG pattern (content-addressable + cryptographic)
  - **5关键洞察**: (1)Memory Poisoning ≠ Prompt Injection — OWASP ASI06 vs LLM01, temporal decoupling makes incident scoping impossible without provenance; (2)Capability-Security tradeoff is real — aggressive write/retrieval policies expand attack surface (Workflow Memory C4 channel is highest-risk); (3)Provenance is the missing primitive in ALL production agent memory systems (Mem0/Letta/Zep lack it) — agent-memory-graph has fingerprint infrastructure to build on; (4)Layered defense > single silver bullet — research consensus across A-MemGuard/VerificAgent/survey; (5)Trust-level DAG propagation: children can't exceed max(parents) trust — consolidation must propagate correctly
  - **npm 差异化**: "Only npm agent memory library with built-in OWASP ASI06 defense" — provenance tracking, trust-aware retrieval, poisoning detection, audit trail. No competing library addresses memory poisoning.
  - **下一步**: agent-memory-graph 添加 source/trust_level/parents[] 字段+quarantine/restore+trace_propagation (~80行+15tests); agent-context-store 添加 detect_injection()+trust_aware_retrieve() (~60行+12tests); README 添加 "OWASP ASI06 compliant" 安全章节

### [06-21 新研究 B] Temporal Knowledge Graphs for Agent Memory: Time-Aware Retrieval, Fact Lifecycle & Evolution
- [x] **Temporal KG 深度研究** ✅ 2026-06-21 晚 — [笔记](catalyst-research/exploration-notes/2026-06-21-temporal-knowledge-graphs-agent-memory.md) ✅ ~200行可运行 TypeScript BiTemporalGraph (bi-temporal edge lifecycle: valid_time + transaction_time, 8/8 assertions pass)
  - **8 papers/systems synthesized**: Zep/Graphiti(arXiv:2501.13956, 212 citations, bi-temporal 4-timestamp model + LLM-driven edge invalidation + 3-tier subgraph hierarchy) + RoMem(arXiv:2604.11544, Semantic Speed Gate relational volatility → continuous phase rotation, SOTA ICEWS05-15 72.6 MRR, 2-3× MRR on temporal reasoning, zero-shot FinTMMBench) + ATOM(EACL 2026, atomic fact decomposition → 5-tuple (s,r,o,t_start,t_end) extraction, ~18% higher exhaustivity, 90% latency reduction) + CIK-LLM(CoLLAs 2024, frozen LLM + dynamic TKG = training-free adaptation) + Graph-based Agent Memory Taxonomy(arXiv:2602.05665, comprehensive survey) + The Missing Knowledge Layer(arXiv:2604.11364, Rust memory-engine with 4-timestamp bi-temporal + Ebbinghaus decay) + TREK(arXiv:2509.15464, 8B matches 671B with temporal reasoning) + Thoughtworks Technology Radar(Graphiti → Trial adoption)
  - **5核心概念**: (1)Bi-Temporal Model — valid_time + transaction_time 双时间线四时间戳; (2)Fact Invalidation Lifecycle — 不删除只标记失效, 保留完整历史; (3)Relational Volatility — RoMem Semantic Speed Gate 关系文本嵌入→波动性评分→连续相位旋转; (4)Hybrid Retrieval — vec+BM25+graph+temporal_filter, 零 query-time LLM; (5)Three-Tier Subgraph — episode/semantic/community 对应人类记忆分类
  - **5关键洞察**: (1)Temporal validity is the missing dimension — agent-memory-graph 缺 valid_from/valid_until/invalidated_by, 3列+~80行即可补齐, 直接对抗 Zep; (2)Invalidation ≠ deletion — TKG的核心哲学差异, 删除丢历史, 追加制造矛盾, 失效保留两者; (3)Relational volatility is the "free" win — volatility_score 字段让 search_hybrid 自动加权时间相关性, npm 无竞品; (4)Production TKG retrieval needs zero LLM calls — 纯 SQL + embedding + BM25 + graph traversal, 验证现有架构; (5)Market gap: SQLite-native temporal KG — Zep 需 Neo4j (重依赖), Python-only; agent-memory-graph 可以是 npm 首个 bi-temporal edge 库
  - **竞品对比**: Zep/Graphiti (Neo4j, Python, Apache 2.0, 20K+ ⭐) / RoMem (research, complex vector rotation) / ATOM (construction-only) / memory-engine (Rust, closest but not npm). agent-memory-graph 差异化: SQLite-embedded + TypeScript-native + 30+ graph algorithms + bi-temporal
  - **下一步**: agent-memory-graph 添加 valid_from/valid_until/invalidated_at/invalidated_by 列 (~3 columns) + invalidate_edge()/query_at()/history()/provenance() (~80行+15 tests) + volatility_score 字段 (~20行+8 tests) + README "Only npm agent memory with bi-temporal validity tracking"

### [06-23 新研究] Graph Reasoning over Agent Memory: From Static Retrieval to Active Inference
- [x] **Graph Reasoning 深度研究** ✅ 2026-06-23 晚 — [笔记](catalyst-research/exploration-notes/2026-06-23-graph-reasoning-agent-memory-active-inference.md) ✅ ~200行可运行 TypeScript GraphReasoner (4 API: reasoning_path + explore + infer_relation + reasoning_subgraph, 5/5 tests pass). **12 papers/systems synthesized**: HopRAG(ACL 2025, retrieve-reason-prune, 36%+ vs dense) + GNN-RAG(ACL 2025, GNN检索+LLM推理, 8.9-15.5% F1) + GR-Agent(arXiv:2512.14766, agent-environment交互不完整知识推理) + SG-RAG(子图检索>传统RAG, 1-3hop显著) + A2RAG(arXiv:2601.21162, cost-aware routing + extraction loss) + GAM(arXiv:2604.12285) + Graph Memory Taxonomy(arXiv:2602.05665) + PathRAG + Agentic Graph RAG(NODES 2026) + ReaGAN(arXiv:2508.00429) + Graph-O1(arXiv:2512.17912) + LEGO-GraphRAG(Cao 2024). **5核心概念**: (1)Retrieve-Reason-Prune三步范式; (2)GNN做检索器+LLM做推理器; (3)Agent-Environment交互式图探索; (4)子图检索信息密度>节点列表; (5)Cost-aware路由(简单→vector, 复杂→graph, 不完整→agentic). **5关键洞察**: (1)图遍历本身就是推理——HopRAG纯遍历(不用LLM)就比BM25高45.84%; (2)不完整知识是常态——agent需要替代路径推理(infer_relation组合link prediction+path finding); (3)推理路径>节点列表——可解释性是生产必需(返回path+explanation); (4)成本感知是GraphRAG生产化门槛(LEGO最优=PPR+small reranker); (5)**npm生态零图推理库**——agent-memory-graph可成为首个. **下一步**: reasoning_path() API ~80行+15tests / explore() API ~60行+12tests / infer_relation() API ~50行+10tests / reasoning_subgraph() API ~40行+10tests / search_graphrag(mode="reasoning") ~40行 / README定位升级为"图推理+...九合一"

### [06-23 新研究 B] Test-Time Scaling for Agent Memory: Adaptive Retrieval Effort & Self-Correcting Search
- [x] **Test-Time Scaling 深度研究** ✅ 2026-06-23 晚 — [笔记](catalyst-research/exploration-notes/2026-06-23-test-time-scaling-adaptive-retrieval-agent-memory.md) ✅ ~360行可运行 TypeScript AdaptiveRetriever (5 components: QueryComplexityClassifier + RetrievalGrader + EvidenceGapTracker + AdmissionController + AdaptiveRetriever, 7/7 tests pass). **15 papers/systems synthesized**: AdaMEM(ICML 2026, arXiv:2606.05684, hybrid long-term trajectory + short-term strategy memory, step-wise test-time adaptation, ALFWorld +13% WebShop +11% positive scaling trend, Step-MFT rejection sampling fine-tuning) + MemR³(arXiv:2512.20237, closed-loop retrieve-reflect-answer with evidence-gap tracker, plug-and-play controller +7.29% on RAG +1.94% on Zep) + A-MAC(ICLR 2026 Workshop MemAgent, arXiv:2603.04549, 5-factor admission scoring U/C/N/R/T, hallucination as first-class, conflict resolution merge) + Compute Allocation(ICLR 2026, arXiv:2603.14635, BM25→Flash-Lite +14.35 NDCG but Flash-Lite→Pro 27× cost for +0.82, diminishing returns cliff) + Adaptive Query Routing(arXiv:2604.14222, 4-tier AHR framework, no single paradigm dominates) + Agent-Orchestrated Adaptive RAG(arXiv:2606.05658, orchestrator 69% standard/16% decomposed, conservative trigger) + CRAG(arXiv:2401.15884, retrieval evaluator Correct/Incorrect/Ambiguous, +20% PopQA) + Self-RAG(ICLR 2024, reflection tokens Retrieve/IsRel/IsSup/IsUse) + SCMRAG(AAMAS 2025, iterative self-corrective multihop, PopQA 76.6%) + Dynamic Cheatsheet(arXiv:2504.07952, test-time learning evolving cheatsheet, Claude 3.5 AIME 6.7%→40.6%, Game of 24 10%→99%) + AdaMem(arXiv:2603.16496, user-centric 4-type memory conditional retrieval) + Adaptive-RAG(NAACL 2024, no/single/multi-step routing, 2026 consensus best practice) + SEAL-RAG(ICLR 2026 withdrawn, entity extraction + fixed-k gap repair) + ICLR 2026 MemAgent Workshop(12+ papers on adaptive memory) + Log-Augmented Generation(scaling test-time reasoning with reusable computation). **5核心概念**: (1)Test-Time Compute Scaling for Memory — AdaMEM positive scaling, Dynamic Cheatsheet test-time learning; (2)Closed-Loop Retrieval — MemR³ retrieve-reflect-answer with evidence-gap tracker; (3)Query Complexity Routing — Adaptive-RAG no/single/multi, 4-tier AHR, cost-aware compute allocation; (4)Self-Correcting Retrieval — CRAG grading + Self-RAG reflection + SCMRAG iterative; (5)Adaptive Memory Admission — A-MAC 5-factor scoring U/C/N/R/T. **5关键洞察**: (1)Memory retrieval is becoming a reasoning process not a lookup — AdaMEM synthesizes strategies online, MemR³ is a sequential decision process; (2)Retrieval LLM augmentation diminishing returns cliff — first level = 95% value, 27× cost for 0.82 points; (3)Evidence-Gap Tracker is the missing primitive for multi-hop reasoning — entity extraction + tag matching, no LLM needed; (4)Adaptive Memory Admission is necessary for production — 5-factor scoring complement to existing consolidation pipeline; (5)**npm生态零自适应检索控制器** — agent-memory-graph可成为首个整合5个方向的自适应检索记忆库. **下一步**: agent-memory-graph Phase 1-2 classify_query()+grade_retrieval() ~110行+27tests → Phase 3 search_with_gaps() ~80行+15tests → Phase 4 should_admit() ~80行+15tests → Phase 5 strategy memory ~50行+10tests. Total ~320行+67tests. README定位升级为 "Adaptive Graph Memory Intelligence Layer"

### [06-24 新研究] MCP Memory Server Protocol: Universal Agent Memory Interface
- [x] **MCP Memory Server 深度研究** ✅ 2026-06-24 晚 — [笔记](catalyst-research/exploration-notes/2026-06-24-mcp-memory-server-protocol-universal-agent-memory-interface.md) ✅ ~300行可运行 TypeScript MCP memory server 原型 (12 MCP tools: remember/recall/update_memory/forget/link/explore/find_path/tag_stats/memory_stats/trace_provenance, MiniGraphMemory class with SQLite + provenance + graph traversal + BM25 search, 11/11 assertions pass)
  - **15 sources synthesized**: memorywire (arXiv:2606.01138v2) 5 ops × 4 types vendor-neutral wire format + MCP TypeScript SDK v1.11.x (Tier 1) + MCP 2026-07-28 RC stateless spec + @modelcontextprotocol/server-memory (75K/wk, JSON file) + Redis Agent Memory Server (Python/FastMCP) + Neo4j NAMS (12 TS tools) + MemoryJS (SQLite bi-temporal) + Cognee MCP (Python Graph-RAG) + MCP adoption 97M monthly / 10K+ servers / Linux Foundation AAIF + Agent Protocol Ecosystem Map (MCP/A2A/ACP/UCP) + Six Agent Protocols 2026 + State of Agent Memory 2026 (Mem0) + LOCOMO benchmarks + MCP vs A2A comparison + Best MCP Servers 2026
  - **5核心概念**: (1)MCP三层协议栈共识 (MCP tools / A2A agents / WebMCP web); (2)memorywire "Memory is not a tool" — 5 ops (remember/recall/update/forget/merge) × 4 types (semantic/episodic/procedural/emotional) + governance channel, plans MCP extension at v0.5; (3)现有 MCP memory server 全部基础存储 (官方=JSON文件 75K/wk / Redis需Python / Neo4j需图DB / 零竞品有图算法+向量+BM25+CRDT); (4)MCP 2026-07-28 stateless spec降低部署门槛 (无session粘性/水平扩展/stdio+HTTP双transport/Full JSON Schema 2020-12); (5)三层产品架构 (Layer1 SDK 334+ APIs → Layer2 memorywire 5 ops → Layer3 MCP 12-15 tools) = "Not just an MCP memory server — a full Graph Intelligence Layer"
  - **5关键洞察**: (1)MCP wrapping memory is lossy — operations→opaque tools, type taxonomy→string param, governance→outside MCP; (2)官方 server-memory 75K周下载但只是JSON文件 — 市场门槛极低, agent-memory-graph是10×差异化; (3)memorywire-compatible标注是战略加分项 — 采用5操作名作为MCP工具名, 提前抢占标准话语权; (4)npm生态零TS-native graph memory MCP server — Redis/Neo4j需外部基础设施, MemoryJS无图算法; (5)三层产品架构清晰化: SDK(完整) → memorywire(标准化) → MCP(消费级子集)
  - **下一步**: README添加"MCP Compatible"+"memorywire-compatible"标注 → 实现agent-memory-graph-mcp包~200行 → 注册MCP Registry → memorywire v0.5对齐

- [ ] **[06-22 新研究 B] Dynamic Community Detection for Evolving Agent Memory Graphs** — [笔记](catalyst-research/exploration-notes/2026-06-22-dynamic-community-detection-evolving-agent-memory.md) ✅ ~200行可运行 TypeScript DF-Leiden (Dynamic Frontier mode: 边变化→affected标记→增量Local Move→社区稳定性追踪, 3场景测试全部通过 Q=0.4462→0.4750). **10 papers/systems synthesized**: Sahu(2024) arXiv:2405.11658 ND/DS/DF三策略 (DS 10-100×, DF 10³× at batch=1) + Lin et al.(Jan 2026) HIT-Leiden 层次增量 10⁵× speedup (2-hop locality bound) + MemGraphRAG(KDD 2026) arXiv:2606.00610 三层全局记忆+社区层次检索 (66.8% vs HippoRAG2 54.4% on 2WikiMultiHopQA) + Traag et al.(2019) original Leiden + Traag et al.(2011) CPM理论基础 + Zarayeneh(2019) Δ-screening + Geraci(2025) Quantum-inspired QICD + Park(2023) CPM well-connectedness下界 + NVIDIA cuGraph(2025) 47.5× GPU加速 + Memgraph 3.8 Atomic GraphRAG. **5核心概念**: (1)动态三策略 ND/DS/DF (affected vertex propagation); (2)HIT-Leiden 层次增量更新 (10⁵× speedup); (3)MemGraphRAG 三层全局记忆 (Schema/Fact/Passage ↔ agent-memory-graph 架构对齐); (4)Affected Vertex Propagation (变化从局部向外扩散); (5)Resolution Stability (社区ID稳定→摘要缓存命中率 90%+). **5关键洞察**: (1)Agent Memory 是动态社区检测完美应用场景 (小批量高频次=DF-Leiden最优工况); (2)社区稳定性是隐藏 Performance Multiplier (Dynamic 90%摘要缓存命中 vs Static 0%→每次$1.00 vs $0.10, 10×成本差); (3)CPM 比 Modularity 更适合动态 Agent Memory (无分辨率极限, γ直接控制粒度, 局部目标函数增量更新更稳定, 推荐 γ=0.1 稀疏图); (4)MemGraphRAG 验证 "Memory+Community+Multi-Agent" 三位一体 (三层架构与 agent-memory-graph 高度对齐); (5)**npm 生态完全没有动态社区检测库** — agent-memory-graph 可成为首个 (graphology 只有静态Louvain, leadenalg 是Python, Neo4j GDS 非npm). **下一步**: 集成静态 Leiden ~190行(已有) → 实现 DF-Leiden update_communities() ~120行 → CPM 质量函数 ~20行 → 社区摘要缓存失效 ~10行 → README "npm 首个支持增量社区更新的图记忆库"
- [ ] **[06-22 新研究 A] LLM-Powered Knowledge Graph Construction for Agent Memory** — [笔记](catalyst-research/exploration-notes/2026-06-22-llm-knowledge-graph-construction-agent-memory.md) ✅ 200+行可运行 TypeScript (3-stage pipeline: extract→resolve→retrieve, char-trigram embedding for entity resolution, BFS multi-hop query, community detection, verified output). **12+ papers/systems synthesized**: arXiv:2602.05665 (Graph Memory Taxonomy) + arXiv:2507.03226 (Practical GraphRAG: dependency parsing 94% of LLM at 0% cost) + Graphiti/Zep (per-episode incremental) + Mem0 (entity collection + AUDN) + Cognee (graph-native) + LightRAG (dual-level indexing) + E2GraphRAG (SpaCy vs LLM) + Jigsaw-LightRAG (delta-only extraction, IOP 2026) + Neo4j NAMS (TypeScript SDK) + MemoryJS (SQLite bi-temporal) + Graphlet AI (entity resolution bottleneck) + Vektor (AUDN write-time curation). **5核心概念**: (1)Three-Pipeline Architecture (Extract→Resolve→Retrieve universal pattern); (2)Incremental vs Batch Construction (Graphiti/Jigsaw delta-only > batch recompute); (3)Entity Resolution 3-tier (exact→alias→embedding, the #1 production pain point); (4)Dual Extraction Mode (LLM deep vs dependency-parser fast, 94% quality at 0% cost); (5)AUDN Write-Time Curation (2026 consensus: heavy write→fast read). **5关键洞察**: (1)**Positioning = Graph Intelligence Layer, NOT Extraction Layer** — bring your own extraction, we provide algorithms + BM25 + vector + CRDT + consolidation all in SQLite; (2)Dependency-based extraction is the enterprise unlock (10-90% cost reduction); (3)agent-memory-graph already has the 5-dim similarity toolkit for entity resolution, missing only `resolve_entities()` orchestrator (~60 lines); (4)AUDN write-time curation is the 2026 consensus architecture; (5)Market gap: only npm library with 30+ graph algorithms + BM25 + vector + CRDT + consolidation + workflow memory in SQLite-embedded TypeScript-native package. **下一步**: resolve_entities() API ~60行+15tests / add_triples() direct-ingestion API ~30行+10tests / README定位为 "Graph Intelligence Layer" / compromise.js轻量级extract_lightweight() ~80行(future)
- [ ] **[06-20 新研究 B] Compositional Agent Memory: 3-Layer Unification** — [笔记](catalyst-research/exploration-notes/2026-06-20-compositional-agent-memory-unified-architecture.md) ✅ ~250行可运行 TypeScript CompositionalMemory (3-layer: episodic Q-value RL scoring + semantic consolidation + procedural skills + AgentFold-inspired context folding + SSGM-inspired drift detection, 4/4 assertions pass). **9 papers/systems synthesized**: MemRL(arXiv:2601.03192) runtime RL on episodic Q-values + MemAct(arXiv:2510.12635) memory-as-action DCPO + AgentFold(arXiv:2510.24699) proactive context folding 30B beats 20x larger + E-mem(ICML 2026) episodic context reconstruction multi-agent + SSGM(arXiv:2603.11768) drift taxonomy + Governed Memory(arXiv:2603.17787) 99.6% fact recall + MemoryArena(ICML 2026) recall≠agency + MLMF(arXiv:2603.29194) adaptive retrieval gating + MemoryAgentBench(ICLR 2026) four core competencies. **核心洞察**: (1)Memory management从系统问题变为学习问题(MemRL/MemAct/AgentFold三条独立路线验证); (2)Stability-Plasticity困境是核心架构张力——frozen LLM + evolving memory是共识; (3)Episodic memory在被重建而非仅仅是检索(E-mem context reconstruction); (4)生产记忆需要治理(SSGM双轨+Governed Memory schema lifecycle); (5)Benchmark从recall进化到agency(MemoryArena: LoCoMo饱和模型在agentic任务骤降). **下一步**: agent-memory-graph添加Q-value utility scoring ~60行+15tests / drift detection API ~50行+10tests / README强调"Compositional 3-Layer Memory"定位
- [ ] **[06-20 新研究 A] Agent Skill Discovery & Reuse: From Workflow Memory to Self-Improving Skill Libraries** — [笔记](catalyst-research/exploration-notes/2026-06-20-agent-skill-discovery-self-improving-libraries.md) ✅ ~200行可运行 TypeScript SkillDiscoveryEngine (failure-driven discovery loop: detect→propose→validate→retrieve→apply, 4/4 assertions pass). Experience Compression Spectrum (arXiv:2604.15877) 统一 memory/skills/rules 为压缩谱系. EvoSkill (Sentient/VT, arXiv:2603.02766) failure-driven discovery + Pareto retention: OfficeQA +7.3%, SealQA +12.1%, cross-task transfer SealQA→BrowseComp +5.3%. SAGE (Amazon, arXiv:2512.17102) RL + Sequential Rollout: +8.9% SGC, −59% tokens. SkillRL (ICLR 2026) recursive skill-augmented RL: 7B beats GPT-4o, 10-20× data compression. MemoryArena (ICML 2026) interdependent multi-session agentic benchmark: LoCoMo-saturating models collapse on agentic tasks. **核心洞察**: (1)Memory→Skill→Rule是压缩谱系不是分离系统; (2)Failure是主要信号(60-75%); (3)SkillRL 10-20×压缩验证procedural memory thesis; (4)Cross-task skill transfer实证; (5)Benchmark从recall→agency. **下一步**: agent-memory-graph 添加 discover_skills() API ~100行 +20 tests / README skill positioning / MemoryArena benchmark
...
## Next Actions (Updated 2026-06-27)

### [06-27 新研究 B] Self-Evolving Graph Memory: ExpGraph + Memory-R1 + Dynamic Leiden
- [x] **Self-Evolving Graph Memory 深度研究** ✅ 2026-06-27 晚 — [笔记](catalyst-research/exploration-notes/2026-06-27-self-evolving-graph-memory.md) ✅ 2 runnable code examples (DynamicLeidenCommunities DF-Leiden incremental update + MemoryRewardTracker RL-style Q-values for ADD/UPDATE/DELETE/NOOP, both verified)
  - **12 papers/systems synthesized**: ExpGraph(UIUC+NTU+Meta, arXiv 2026.05, self-evolving experience graph + diffusion retrieval + RL copilot, +21.4% agentic) + Memory-R1(arXiv 2025.08, RL-learned memory CRUD from ~150 examples, +68.9% F1, “most important development of 2025-2026”) + DF-Leiden(arXiv:2405.11658, Dynamic Frontier 1.98× speedup) + HIT-Leiden(Lin Jan 2026, hierarchical bounded 105× speedup) + Graph-based Agent Memory Survey(arXiv:2602.05665, taxonomy: layers/traversal/RL operators) + A-Mem(NeurIPS 2025, Zettelkasten agentic memory) + From Experience to Strategy(ICLR 2026, trainable graph memory RL weights) + MEM1(ICLR 2026, RL constant-context long-horizon) + SemToG(Mishra 2025, semantic community detection) + MemoryArena(He 2026, recall≠agency 90%→40-60%) + Mem0 State of Memory 2026(186M API calls, 48K stars) + Graphiti/Zep(production HybridRAG)
  - **5核心概念**: (1)Graph Diffusion Retrieval > Pure Vector — ExpGraph diffusion captures multi-hop context that vector search misses; (2)Memory-R1 Learned CRUD — ADD/UPDATE/DELETE/NOOP policy from outcome rewards, no hand-written rules; (3)Dynamic Leiden — DF-Leiden incremental community update only re-evaluates affected frontier, 1.98×-105× speedup; (4)Hierarchical Multi-Layer Graph — raw→summarized→strategy layers with inter-layer traversal (G-Memory/LiCoMemory/Trainable); (5)Benchmark-to-Deployment Gap — 90% recall on LoCoMo → 40-60% agentic performance
  - **5关键洞察**: (1)Memory management is becoming a learned skill not hand-written rules — Q-value scoring item is stepping stone toward Memory-R1; (2)Graph diffusion is the missing link between vector search and graph traversal — replaces fixed-hop BFS; (3)DF-Leiden makes community detection practical for streaming memory — every insertion without performance concern; (4)agent-memory-graph positioned as bridge between production (Mem0/Zep) and research (ExpGraph/Memory-R1); (5)Need agentic evaluation suite — stop optimizing recall only, measure task success
  - **5下一步**: (1)DF-Leiden integration ~190 lines (last major agent-memory-graph feature); (2)diffusion_retrieve() personalized PageRank ~150 lines+30 tests; (3)MemoryRewardTracker → Q-value scoring ~100 lines+20 tests; (4)Agentic evaluation in MemoryBenchmarkHarness; (5)README positioning as 'Bridge between production and research agent memory'

### [06-27 新研究 A] Bi-Temporal Agent Memory: Validity Tracking, Invalidation & Time-Travel Queries
- [x] **Bi-Temporal 深度研究** ✅ 2026-06-27 晚 — [笔记](catalyst-research/exploration-notes/2026-06-27-bi-temporal-agent-memory-validity-invalidation-time-travel.md) ✅ ~300行可运行 TypeScript BiTemporalMemoryStore (deterministic supersession + Type I/II invalidation + cascade rules + time-travel query + restore + audit trail, 16/16 assertions pass)
  - **12 papers/systems synthesized**: MemStrata(arXiv:2606.26511, deterministic (S,R,O) supersession, stale-fact ~0%, cosine AUROC=0.59 impossibility result) + STALE(arXiv:2605.06527, 400 conflict scenarios, Type I co-referential vs Type II propagated/cascading invalidation, CUPMem write-side adjudication) + TSM(arXiv:2601.07468, Temporal Semantic Memory, TKG with validity intervals, 74.8% vs Mem0 53.6%) + Graphiti/Zep(arXiv:2501.13956, bi-temporal timeline_valid+transaction, 20K+ stars, sub-200ms p95) + LedgerRAG(MDPI Electronics 2026, evidence ledger with temporal validity signals) + Temporal Evidence Chain(ACL 2026) + Temporal AI Agents(Cobus Greyling, multi-agent invalidation pipeline) + TigerData/pgvectorscale(PostgreSQL bi-temporal schema production guide) + Atlan framework comparison(Zep temporal invalidation best, Mem0/Letta/LangChain none) + Oracle Agent Memory + Zhang&Choi EMNLP 2023 + SQL:2011 standard
  - **5核心概念**: (1)Bi-Temporal Model — valid_time (world) + transaction_time (system) = two axes per fact; (2)Embedding CANNOT detect staleness — MemStrata proved cosine AUROC=0.59, contradictions MORE similar than duplicates; (3)Invalidation Taxonomy — Type I direct vs Type II cascade (STALE showed ~30% accuracy drop for cascading); (4)Retire-Don't-Delete — superseded facts kept for audit/time-travel, not deleted; (5)Multi-Agent Temporal Pipeline — Extraction→Temporal→Invalidation→Retrieval agents
  - **5关键洞察**: (1)Vector similarity ≠ staleness detector — deterministic (S,R,O) supersession rules needed, consolidation_pipeline is complementary not substitute; (2)Write-time > read-time for staleness — STALE proved recognition ≠ application, fix at assert() not query(); (3)Graphiti is Python-only, npm has ZERO bi-temporal agent memory — agent-memory-graph greenfield; (4)Cascade invalidation is unsolved differentiator — no competitor has rule-based cascade; (5)Bi-temporal + HLC = full accountability layer (validity ordering + causal ordering)
  - **Phase 1-4 plan**: P1 core bi-temporal ~80行+15tests (valid_from/valid_until/invalidated_by columns + assert_with_validity + query_as_of + supersession_chain) / P2 cascade rules ~60行+12tests / P3 README citing MemStrata+STALE+Graphiti / P4 advanced (find_stale + temporal_diff + bi-temporal CRDT merge)

### [06-26 新研究] KV Cache as Agent Working Memory: From Inference Optimization to Cross-Agent Collaboration
- [x] **KV Cache 深度研究** ✅ 2026-06-26 晚 — [笔记](catalyst-research/exploration-notes/2026-06-26-kv-cache-as-agent-working-memory-inference-to-collaboration.md) ✅ ~300行可运行 TypeScript KVCacheAwareMemoryManager (cache temperature scoring + workflow-aware eviction + cross-agent segment reuse + plan template extraction + health report, 5/5 assertions pass)
  - **15+ papers/systems synthesized**: KVFlow(NeurIPS 2025, arXiv:2507.07400) + KEEP(DAC 2026, arXiv:2602.23592) + Segment-Level KV Cache Sharing(ICLR 2026 OpenReview kgzBkyqg6Z) + TokenDance(arXiv:2604.03143, cites OpenClaw) + SideQuest(NVIDIA, arXiv:2602.22603) + KVCOMM(NeurIPS 2025) + Continuum(arXiv:2511.02230) + Agentic Plan Caching(NeurIPS 2025, arXiv:2506.14852) + AgenticCache(arXiv:2604.24039) + IntentKV(arXiv:2606.09916) + Tokencake + CacheSlide(FAST 2026) + ScaleSim + SIGARCH 2026 + LLM Agent Memory Survey + Semantic Sponsorship(arXiv:2604.11288) + Persistent Q4 KV Cache(arXiv:2603.04428)
  - **5核心概念**: (1)KV Cache IS Agent Working Memory — not just inference optimization; (2)Prefix Barrier Breaking — 3 independent approaches (Segment-Level/KVCOMM/TokenDance); (3)Model-Driven Eviction — LLM-as-garbage-collector > heuristics; (4)Workflow-Aware Cache — Step Graph + TTL + Plan Templates; (5)SIGARCH Cache Coherence for Agents — agent-memory-graph CRDT+HLC addresses Missing Piece 1
  - **5关键洞察**: (1)External memory ↔ KV cache = two layers of same problem; (2)Prefix Barrier = BM25 Barrier (same evolution); (3)KVFlow Step Graph maps to Workflow Memory; (4)Plan Caching = inference-layer Workflow Memory (50% cost ↓); (5)npm ecosystem zero TS-native KV cache bridges = greenfield
  - **5下一步**: (1)agent-memory-graph cache_temperature() API ~40行+10tests; (2)plan template extraction ~60行+15tests; (3)agent-context-store KV-cache-aware TTL ~30行+8tests; (4)README 'KV Cache Aware' section + SIGARCH positioning; (5)future agent-kv-bridge package ~200行

### [06-25 新研究] Vector Clocks → HLC: Multi-Agent Memory Synchronization
- [x] **HLC + Subscribe 深度研究** ✅ 2026-06-25 晚 — [笔记](catalyst-research/exploration-notes/2026-06-25-vector-clocks-hlc-multi-agent-sync.md) ✅ 3 runnable code examples (HLC TypeScript impl + filtered subscribe Python + DVV pruning diff)
  - **7 sources synthesized**: Kulkarni HLC paper (2014) + Shapiro CRDT foundations (2011) + Riak ORSWOT delta-CRDT (arXiv:1605.06424) + Confluent EDA for Agents (2025) + TypeOnce HLC TypeScript + OneUptime VC guide + Ian Duncan CRDT Dictionary
  - **5 core concepts**: (1)Vector Clock O(N) vs HLC O(1) causal tracking; (2)OR-Set add-wins semantics for concurrent operations; (3)Event-Driven Blackboard Pattern (Confluent 2025 — subscribe = local Kafka consumer); (4)Dotted Version Vectors (DVV) prune zero entries for 5-line compression; (5)HLC enables cross-cluster agent sync (vs VC lossy rollup)
  - **5 key insights**: (1)HLC > VC at 20+ agents scale — 50× space compression, same causal guarantees; (2)Subscribe needs filtering for multi-agent scalability (O(N²) without it); (3)Replay-based subscribe = event sourcing for agent memory (offline→replay→live, no Kafka); (4)OR-Set merge exists but bypasses VC — concurrent causal check missing; (5)HLC enables MDC sync (matters for A2A trust prototype)
  - **4 next steps**: (1)subscribe_filtered() + unsubscribe() ~40行; (2)DVV pruning in _vector_clock_increment() ~3行; (3)HLC mode as alternative clock ~60行; (4)Wire apply_changes() HLC comparator ~15行. Total ~118行 + 15-20 tests
  - **Builds on**: agent-memory-graph CRDT research (06-16), MCP Memory Server Protocol (06-24), A2A Trust Prototype pending task

### [06-24 新研究 C] Agent Memory Benchmarks & Evaluation 2026: From Recall to Agency
- [x] **Agent Memory Benchmarks 深度研究** ✅ 2026-06-24 晚 — [笔记](catalyst-research/exploration-notes/2026-06-24-agent-memory-benchmarks-evaluation-2026.md) ✅ ~300行可运行 TypeScript MemoryBenchmarkHarness (3-layer evaluation: accuracy + efficiency + difficulty, system-to-system comparison, 7/7 assertions pass)
  - **15+ benchmarks/systems synthesized**: LoCoMo (NeurIPS 2024, 1540 Q) + LongMemEval (500 Q) + BEAM (ICLR 2026, 2000 Q, 10 abilities, 128K-10M) + MemoryArena (ICML 2026, agentic task completion) + MemoryAgentBench (ICLR 2026, 4 competencies) + Awesome-Agent-Memory benchmark list (20+ benchmarks catalogued) + Mem0/Zep/Letta/Hindsight/LangMem/SuperMemory production comparison matrix + DeepEval 6 metrics + Vectorize AMB + Letta filesystem 74% finding + LoCoMo Refined/Plus + StreamMemBench/AMemGym/RoboMemArena/Evo-Memory/OdysseyBench
  - **5核心概念**: (1)Three Benchmark Pillars LoCoMo/LongMemEval/BEAM覆盖10K-10M token; (2)MemoryArena recall≠agency (90% LoCoMo → 40-60% agentic); (3)MemoryAgentBench 4 competencies (Accurate Retrieval / Test-time Learning / Long-range Understanding / Information Consolidation); (4)Multi-Dimensional Evaluation (accuracy + token efficiency ~7K optimal + retrieval latency p50/p95 + trajectory efficiency + memory ops ratio); (5)Benchmark Saturation (LoCoMo 90%+ solved, BEAM-10M <50% unsolved, new frontiers: streaming/robotic/evolving)
  - **5关键洞察**: (1)Recall benchmarks solved, agency benchmarks not — README should position beyond recall, pitch graph reasoning+consolidation+workflow memory; (2)BEAM-10M是new competitive frontier, no system >50%, state integrity breaks at 500K not 10M — agent-memory-graph graph+temporal+consolidation addresses this; (3)Three-layer evaluation (accuracy/efficiency/agentic) is 2026 consensus — DeepEval formalizes as end-to-end+trajectory+component; (4)Memory ops ratio is hidden health metric (NOOP >50% = healthy, agent-memory-graph already has memory_audit); (5)Benchmark manipulation is real (Letta filesystem 74%, MemPalace debunked, LoCoMo Refined 20-30% scoring changes) — README must include methodology+tokens+latency
  - **Production Results Matrix**: Zep 94.7% LoCoMo / 155ms / 5.8K tokens (best combined) > Mem0 92.5% LoCoMo / 94.4% LongMemEval (best LongMemEval) > Hindsight 91.4% / 64.1% BEAM-10M (best at 10M) > Letta 74-83% > LangMem 58.1% (p95=59.8s, impractical) > Full-context highest accuracy but 14× cost, 17s p95
  - **3下一步**: (1)agent-memory-graph README添加benchmark positioning section (定位为"beyond recall — agentic graph memory"); (2)实现MiniBenchmarkHarness集成到lab/ (~200行, 可选GPT-4o judge); (3)npm发布时标注"Supports agentic memory patterns beyond LoCoMo/LongMemEval recall paradigm"
...
### 2026-06-22
- ✅ **agent-context-store key-dev 2-cycle 凌晨** — 1541→1569 (+28, Cycle 148): tag_similarity_matrix (normalized tag co-occurrence matrix, jaccard/cosine/raw modes) + content_context_expand (multi-signal fusion: tag_overlap 0.35 + xref 0.25 + embedding 0.40) + store_dashboard (unified one-call overview aggregating vitality+security+freshness+tags+compaction+depth+recommendations). Zero regressions
- ✅ **agent-context-store key-dev 3-cycle 凌晨** — 1569→1598 (+29, Cycle 149): tag_recommend (predictive: TF-IDF keywords + similarity matrix co-occurrence + existing-tag expansion) + content_semantic_search (TF-IDF cosine + tag relevance boost 0.7/0.3, 4-tier search COMPLETE: exact→fuzzy→semantic→embedding) + store_diff (cross-store comparison: added/removed/modified, embedding drift, multi-agent sync preview). Zero regressions
- ✅ **context-forge tool-development evening** — 131→235 (+104): F7 TOML/YAML structured export (zero-dep serializers, +28) + F13 template registry system (register/get/list/remove, 3 built-in templates, +39) + F16 integration tests (self-analysis, temp project lifecycle, cross-format validation, +13). Zero regressions
- 连续146天零回滚率 🏆
- **四项目测试总量突破 4279** (agent-memory-graph 1307 + agent-context-store 1652 + structured-output-toolkit 438 + agent-task-cli 882)
- **agent-context-store API 总量突破 440 methods**
- **context-forge 突破 402 tests** (06-22: F17-F19 + F28-F30, complexity analysis/project comparison/health scoring/TODO scanning/env var detection/license detection)

### 2026-06-21
- ✅ **agent-context-store key-dev 2-cycle 凌晨** — 1404→1428 (+24, Cycle 141): tag_auto_label (auto-propose+apply tags from TF-IDF, closes tag-hygiene loop: report→audit→**fix**) + content_compaction_report (ROI estimate for merging: entries_removable/bytes_saved/saving_pct/recommendation) + embedding_outlier_pairs (outlier→nearest-core pairing for thematic drift detection). Zero regressions
- ✅ **agent-context-store key-dev 3-cycle 凌晨** — 1428→1454 (+26, Cycle 142): tag_auto_label_batch (batch auto-labelling across all low-alignment entries, auto-select from tag_audit_all<0.3) + content_deduplicate (actual merge execution: canonical append+tag migration+delete, dry_run safety) + embedding_topic_clusters (k-means++ themantic clustering with coherence/separation/silhouette). **三大分析管线全部 COMPLETE**. Zero regressions
- 连续142天零回滚率 🏆
- **四项目测试总量突破 4081** (agent-memory-graph 1307 + agent-context-store 1454 + structured-output-toolkit 438 + agent-task-cli 882)

### 2026-06-20
- ✅ **agent-context-store key-dev 2-cycle 凌晨** — 1360→1379 (+19, Cycle 139): tag_audit_all + content_duplicate_graph + embedding_core_sample. Zero regressions
- ✅ **agent-context-store key-dev 3-cycle 凌晨** — 1379→1404 (+25, Cycle 140): tag_health_report + content_merge_suggestions + embedding_density_map. **Analytics executive layer 完成**. Zero regressions
- ✅ **context-forge 晚间** — 84→109 (+25): fix 3 test bugs + F1 git history analysis (8 tests) + F12 diff preview (15 tests). Zero regressions
- ✅ **深度研究: Compositional Agent Memory** — 9 papers synthesized (MemRL/MemAct/AgentFold/E-mem/SSGM/Governed Memory/MemoryArena/MLMF/MemoryAgentBench), ~250行 TS CompositionalMemory 4/4 pass
- ✅ **技术随笔发表** — "好执行者不是好老师：Agent Workflow Memory 的三个工程陷阱" [link](https://robertsong2019.github.io/posts/agent-workflow-memory-trajectory-to-skill-2026-06.html)
- 连续140天零回滚率 🏆
